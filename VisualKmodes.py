...
def heatmap_correlazioni():
    #Creazione di una copia per evitare di sostituire valori originali
    df_encoded = df.copy()
    ...
    corr = df_encoded.corr() #Calcolare matrice di correlazione tra le colonne    
           

    plt.figure(figsize=(14,12)) #Creazione una nuova figura  
    
    #Creazione della heatmap con Seaborn
    sns.heatmap(corr, annot=True, fmt=".2f", cmap="viridis", linewidths=0.5, linecolor="gray", cbar_kws={"shrink":0.8}, annot_kws={"size":10})         
    plt.title("Heatmap delle correlazioni tra le variabili dei funghi", fontsize=16, fontweight="bold") 
    plt.xticks(rotation=45, ha="right", fontsize=10)
    plt.yticks(fontsize=10)
    plt.tight_layout() #Ottimizzazione layout
    plt.show()              
...
def k_modes_cluster(n_clusters=2):
    ...
    km = KModes(n_clusters=n_clusters, init='Huang', n_init=5, verbose=0)   #verbose:Disattiva messaggi di log
